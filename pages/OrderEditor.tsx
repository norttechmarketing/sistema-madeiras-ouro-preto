...
import { validateAndRefreshSession } from '../lib/supabase';
...
const OrderEditor: React.FC = () => {
  ...
  const saveOrder = async (silent = false, typeOverride?: 'Orçamento' | 'Pedido') => {
    ...
    // Validar sessão ativa antes de salvar
    const sessionActive = await validateAndRefreshSession();
  ...
  const handleConvertToOrder = async () => {
    ...
    // Validar sessão ativa antes de iniciar a conversão
    const sessionActive = await validateAndRefreshSession();
    if (!sessionActive) return;

    if (!orderItems.length) {
      alert("Adicione ao menos um item antes de converter.");
      return;
    }
    ...
    setIsSaving(true);
    try {
      // 1. Salvar alterações atuais como Orçamento antes de converter, garantindo dados íntegros e ID na base
      const savedDoc = await saveOrder(true, 'Orçamento');
      if (!savedDoc) throw new Error("Não foi possível salvar o orçamento antes da conversão.");

      // 2. Converter o orçamento atual em pedido (mesmo ID, move de tabela)
      ...
      // 3. Atualizar estado local de tipo de documento
      setOrderType('Pedido');

      // 4. Carregar os dados atualizados para garantir sincronia completa e evitar estados inconsistentes
      await loadOrderData(activeOrderId);

      alert('Orçamento convertido em Pedido com sucesso!');

      // 5. Atualizar rota para incluir o ID correto e remover parametro ?type
  ...
  const handleWhatsApp = async () => {
    ...
    // Validar sessão ativa antes do envio
    const sessionActive = await validateAndRefreshSession();
    if (!sessionActive) return;
    ...
    try {
      ...
      // Se for novo ou tiver mudanças (simulado), salva.
  ...
  const handlePrint = async () => {
    ...
    // Validar sessão ativa antes de gerar PDF
    const sessionActive = await validateAndRefreshSession();
    if (!sessionActive) return;
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-1">
                            <button
                              ...
                              className={`text-[10px] font-black min-w-[28px] h-[48px] rounded-xl transition-all border flex items-center justify-center ${item.discountType === 'percentage'
